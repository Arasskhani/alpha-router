...
from app.services.nitro_api_key_audit import (
...
from app.services.nitro_api_key_service import (
    apply_expiration,
    compute_expires_at,
    key_to_dict,
...
async def list_nitro_keys(
    ...
    stmt = select(NitroApiKey)
    if q:
        stmt = stmt.where(NitroApiKey.name.ilike(f"%{q.strip()}%"))
    if owner:
        needle = f"%{owner.strip()}%"
        stmt = stmt.join(User, NitroApiKey.owner_user_id == User.id).where(
            or_(
    ...
    rows = (
        await db.execute(
            stmt.order_by(NitroApiKey.created_at.desc())
...
async def toggle_nitro_key(
    key_id: int,
    enabled: bool,
    db: AsyncSession = Depends(get_db),
    admin: User = Depends(require_api_keys_write),
):
    k = await db.get(NitroApiKey, key_id)
    if not k:
        raise HTTPException(404)
...
async def delete_nitro_key(key_id: int, db: AsyncSession = Depends(get_db), _: User = Depends(require_api_keys_write)):
    k = await db.get(NitroApiKey, key_id)
    if not k:
        raise HTTPException(404)
    await db.delete(k)
...
async def bulk_nitro_keys(
    ...
    ids = list(dict.fromkeys(body.ids))
    if action == "delete":
        result = await db.execute(delete(NitroApiKey).where(NitroApiKey.id.in_(ids)))
        await db.commit()
        return {"ok": True, "count": result.rowcount or 0}
    enabled = action == "on"
    rows = (await db.execute(select(NitroApiKey).where(NitroApiKey.id.in_(ids)))).scalars().all()
    changed = 0
    for k in rows:
        if k.is_active == enabled:
            continue
...
async def _fetch_logs_since(
    ...
    *,
    ...
    nitro_api_key_id: int | None = None,
    ...
    if nitro_api_key_id is not None:
        q = q.where(RequestLog.nitro_api_key_id == nitro_api_key_id)
...
async def dashboard_activity_export(
    ...
    return await _activity_export_response(
        db,
        format=format,
        filename_stem=f"nitro-activity-{period}-{group_by}",
        scope="service",
        jwt_token=jwt_token,
...
async def api_key_activity(
    ...
    if pp not in ("day", "week", "month"):
        pp = "week"
    key = await db.get(NitroApiKey, key_id)
    if not key:
...
async def api_key_activity_export(
    ...
    return await _activity_export_response(
        db,
        format=format,
        filename_stem=f"nitro-api-key-{key_id}-activity-{period}",
        scope="api_key",
        jwt_token=jwt_token,
        user_role=admin.role,
        period=period,
...
async def connection_activity_export(
    ...
    return await _activity_export_response(
        db,
        format=format,
        filename_stem=f"nitro-connection-{conn_id}-activity-{period}",
        scope="connection",
        jwt_token=jwt_token,
        user_role=admin.role,
        period=period,
...
async def user_activity_export(
    ...
    return await _activity_export_response(
        db,
        format=format,
        filename_stem=f"nitro-user-{user_id}-activity-{period}",
        scope="user",
        jwt_token=jwt_token,
        user_role=admin.role,
        period=period,
...
async def admin_user_media_download_zip(
    ...
    filename = f"nitro-media-{safe_user}-{stamp}.zip"
    return Response(
        content=data,
