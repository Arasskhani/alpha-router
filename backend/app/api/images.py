...
_NITRO_MEDIA_PATH = re.compile(r"/api/chat/media/(\d+)/file/?(?:\?.*)?$")


def parse_nitro_media_asset_id(reference: str) -> int | None:
    """Extract media asset id from NITRO /api/chat/media/{id}/file URLs."""
    ref = (reference or "").strip()
    if not ref:
        return None
    path = urlparse(ref).path if ref.startswith(("http://", "https://")) else ref.split("?")[0]
    match = _NITRO_MEDIA_PATH.search(path)
    if not match:
        return None
    try:
...
async def resolve_reference_image_for_upstream(
    ...
    """Turn NITRO media paths into data URLs OpenRouter and other providers can consume."""
    ...
    asset_id = parse_nitro_media_asset_id(ref)
