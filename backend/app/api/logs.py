...
def _log_row(
    ...
    *,
    ...
    elif source_code in ("nitro_chat", "billi_chat"):
        identity_type = "chat"
    else:
        identity_type = "user"
    row = {
    ...
    if nitro_key:
        row["api_key_name"] = nitro_key.name
        row["api_key_prefix"] = nitro_key.key_prefix
        row["nitro_api_key_id"] = nitro_key.id
    return row
...
def _apply_log_filters(
    ...
    *,
    ...
    if username:
        term = username.strip()
        key_match = select(NitroApiKey.id).where(NitroApiKey.name.contains(term))
        q = q.where(
            or_(
                RequestLog.username.contains(term),
                RequestLog.nitro_api_key_id.in_(key_match),
            )
        )
    if model_id:
...
async def admin_logs_filter_options(
    ...
    api_keys = (
        await db.execute(
            select(NitroApiKey.name)
            .join(RequestLog, RequestLog.nitro_api_key_id == NitroApiKey.id)
            .where(NitroApiKey.name.isnot(None), NitroApiKey.name != "")
            .distinct()
            .order_by(NitroApiKey.name)
        )
    ).scalars().all()
...
async def admin_logs(
    ...
    return {
        "items": [
            _log_row(r, provider_map.get((r.model_id or "").strip()), nitro_key=key_map.get(r.nitro_api_key_id))
            for r in rows
        ],
        "offset": offset,
        "limit": limit,
