...
def _report_params(body: ReportRequest) -> dict:
    ...
    params: dict = {
        ...
        "nitro_api_key_id": body.nitro_api_key_id,
...
async def report_options(db: AsyncSession = Depends(get_db), _: User = Depends(require_reports)):
    ...
    keys = (
        await db.execute(select(NitroApiKey.id, NitroApiKey.name).order_by(NitroApiKey.name))
    ).all()
    ...
    return {
        ...
        "nitro_api_keys": [{"id": k[0], "name": k[1]} for k in keys],
        "auth_providers": ["local", "ldap", "keycloak", "openwebui"],
        "group_by_options": [
