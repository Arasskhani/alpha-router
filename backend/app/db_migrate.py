...
async def apply_branding_migrations() -> None:
    """Rename legacy tables/values to Alpha Router (once)."""
    async with engine.begin() as conn:
        ...
        def migrate(connection) -> None:
            ...
            insp = inspect(connection)
            tables = set(insp.get_table_names())
            if "alpha_router_api_keys" in tables and "alpha_router_api_keys" not in tables:
                connection.execute(text("ALTER TABLE alpha_router_api_keys RENAME TO alpha_router_api_keys"))
            if "request_logs" in tables:
                ...
                connection.execute(
                    text(
                        "UPDATE request_logs SET user_id = NULL WHERE alpha_router_api_key_id IS NOT NULL"
                    )
                )
                if "alpha_router_api_keys" in tables:
                    connection.execute(
                        text(
                            """
                            UPDATE request_logs
                            SET username = (
                                SELECT name FROM alpha_router_api_keys
                                WHERE alpha_router_api_keys.id = request_logs.alpha_router_api_key_id
                            )
                            WHERE alpha_router_api_key_id IS NOT NULL
                            AND (
                                username IS NULL
                                OR username = ''
            ...
            if "users" in tables and "request_logs" in tables:
                ...
                connection.execute(
                    text(
                        ...
                        AND id NOT IN (
                            SELECT owner_user_id FROM alpha_router_api_keys
