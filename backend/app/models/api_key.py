"""NITRO gateway keys (admin) and per-user keys."""
...
class NitroApiKey(Base):
    ...
    audit_logs = relationship("NitroApiKeyAuditLog", back_populates="api_key", cascade="all, delete-orphan")


class NitroApiKeyAuditLog(Base):
    """Who changed a gateway API key and which fields changed."""

    __tablename__ = "nitro_api_key_audit_logs"

    id = Column(Integer, primary_key=True)
    nitro_api_key_id = Column(Integer, ForeignKey("nitro_api_keys.id", ondelete="CASCADE"), index=True)
    actor_user_id = Column(Integer, ForeignKey("users.id", ondelete="SET NULL"), nullable=True, index=True)
    action = Column(String(32), nullable=False)
    changes_json = Column(Text, nullable=False, default="[]")
    created_at = Column(DateTime, default=datetime.datetime.utcnow, index=True)

    api_key = relationship("NitroApiKey", back_populates="audit_logs")
