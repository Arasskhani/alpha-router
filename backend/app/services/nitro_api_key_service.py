...
async def maybe_reset_key_period(db: AsyncSession, key: NitroApiKey) -> None:
    now = _utc_now()
...
def apply_expiration(key: NitroApiKey) -> bool:
...
async def ensure_key_usable(db: AsyncSession, key: NitroApiKey) -> None:
...
async def record_key_usage(db: AsyncSession, key: NitroApiKey, cost_usd: float) -> None:
    if cost_usd <= 0:
        return
    await maybe_reset_key_period(db, key)
