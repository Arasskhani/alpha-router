...
Costs are taken from provider usage objects — never adjusted by NITRO.
"""
...
async def preflight_stream_chat(
    ...
    *,
    ...
    if nitro_api_key_id:
        ...
        nitro_key = await db.get(NitroApiKey, nitro_api_key_id)
        if nitro_key:
...
async def log_usage(
    ...
    *,
    ...
    if nitro_api_key_id and total_cost_usd > 0:
        from app.models.api_key import NitroApiKey
        from app.services.nitro_api_key_service import record_key_usage

        key = await db.get(NitroApiKey, nitro_api_key_id)
        if key:
...
async def stream_chat(
    ...
    *,
    ...
    async with AsyncSessionLocal() as db:
        ...
        persister = None
        if source == "nitro_chat" and user_id:
