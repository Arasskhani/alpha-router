...
async def report_nitro_api_key_usage(
    db: AsyncSession, start: datetime, end: datetime, nitro_api_key_id: int | None
) -> pd.DataFrame:
    q = (
        select(
            RequestLog.nitro_api_key_id,
            func.sum(RequestLog.total_cost_usd),
            func.count(),
        )
        .where(
            RequestLog.request_time >= start,
            RequestLog.request_time <= end,
            RequestLog.nitro_api_key_id.isnot(None),
        )
        .group_by(RequestLog.nitro_api_key_id)
        .order_by(func.sum(RequestLog.total_cost_usd).desc())
    )
    if nitro_api_key_id:
    ...
    for key_id, cost, count in rows:
        key = await db.get(NitroApiKey, int(key_id)) if key_id else None
        data.append(
            {
...
async def report_nitro_api_keys_near_credit_limit(db: AsyncSession, threshold_pct: float) -> pd.DataFrame:
    keys = (await db.execute(select(NitroApiKey).where(NitroApiKey.is_active == True))).scalars().all()  # noqa: E712
    rows = []
    for k in keys:
...
async def build_report(db: AsyncSession, report_type: str, params: dict[str, Any]) -> pd.DataFrame:
    ...
    if report_type == "nitro_api_key_usage":
        return await report_nitro_api_key_usage(db, start, end, params.get("nitro_api_key_id"))
    if report_type == "nitro_api_keys_near_credit_limit":
        return await report_nitro_api_keys_near_credit_limit(db, float(params.get("threshold_pct") or 80))
