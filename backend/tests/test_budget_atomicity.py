...
nitro API key usage counters, including NULL handling and concurrent runs.
"""
...
async def _bootstrap():
    ...
    async with factory() as db:
        ...
        key = (await db.execute(select(NitroApiKey))).scalar_one()
...
async def _test_atomic_key_increment_sums_correctly() -> None:
    ...
    async def add_cost():
        async with factory() as db:
            key = await db.get(NitroApiKey, kid)
    ...
    async with factory() as db:
        await db.execute(
            __import__("sqlalchemy").text(
                "UPDATE nitro_api_keys SET period_used_usd = NULL, total_used_usd = NULL WHERE id = :kid"
            ),
            {"kid": kid},
        )
        await db.commit()
