...
def test_run_sandbox_uses_container_when_image_set(monkeypatch):
    ...
    assert out == "cont-ok"
    assert captured["image"] == "alpha-router-sandbox:latest"
...
def _sandbox_image_available() -> bool:
    if not shutil.which("docker"):
        return False
    try:
        r = asyncio.run(_docker_image_exists("alpha-router-sandbox:latest"))
    except Exception:
        return False
    return r
...
_DOCKER = pytest.mark.skipif(
    ...
    reason="alpha-router-sandbox:latest not available or docker missing",
)
