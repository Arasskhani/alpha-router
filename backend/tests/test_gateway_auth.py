...
- Nitro API key skips user budget but is still subject to its own credit limit.
...
async def _seed_user_key(session_factory, raw_key="nitro_userkey_test123"):
...
async def _seed_nitro_key(session_factory, *, raw_key="nitro_nitrokey_test456",
...
async def _test_user_api_key_resolves_owner_with_budget():
    ...
    async with sf() as db:
        ...
        assert auth.skip_budget is False
        assert auth.nitro_api_key_id is None
        assert auth.user_id is not None


async def _test_nitro_api_key_skips_user_budget():
    _, sf = await _setup_db()
    _, raw = await _seed_nitro_key(sf, credit_limit_usd=0.0)
    async with sf() as db:
        auth = await gateway._resolve_gateway_auth(_FakeRequest(f"Bearer {raw}"), db, {})
        assert auth.source == "nitro_key"
        assert auth.skip_budget is True
        assert auth.nitro_api_key_id is not None
        assert auth.user_id is None


async def _test_nitro_api_key_over_credit_limit_raises_402():
    _, sf = await _setup_db()
    _, raw = await _seed_nitro_key(sf, credit_limit_usd=1.0, period_used_usd=1.0)
    async with sf() as db:
        with pytest.raises(HTTPException) as exc:
...
def test_nitro_api_key_skips_user_budget():
    asyncio.run(_test_nitro_api_key_skips_user_budget())


def test_nitro_api_key_over_credit_limit_raises_402():
    asyncio.run(_test_nitro_api_key_over_credit_limit_raises_402())
