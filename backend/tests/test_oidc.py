...
def test_validate_realm_rejects_path_injection():
    with _patch_settings():
        ...
        assert oidc.validate_realm("nitro") == "nitro"
...
def test_validate_id_token_accepts_well_formed():
    ...
    issuer = "https://kc.example.com/realms/nitro"
    nonce = "nonce-123"
    claims = {
        "iss": issuer,
        "aud": "nitro-ui",
    ...
    with _patch_settings():
        with patch("app.services.oidc._fetch_jwks", return_value=jwks):
            out = oidc.validate_id_token(token, issuer=issuer, audience="nitro-ui", nonce=nonce)
...
def test_validate_id_token_rejects_wrong_issuer():
    pem, jwks = _make_keypair_and_jwks()
    claims = {
        "iss": "https://kc.example.com/realms/nitro",
        "aud": "nitro-ui",
        "exp": int(time.time()) + 300,
        "sub": "x",
        "nonce": "n",
    ...
    with _patch_settings():
        with patch("app.services.oidc._fetch_jwks", return_value=jwks):
            with pytest.raises(ValueError):
                oidc.validate_id_token(token, issuer="https://kc.example.com/realms/other", audience="nitro-ui", nonce="n")


def test_validate_id_token_rejects_wrong_audience():
    pem, jwks = _make_keypair_and_jwks()
    issuer = "https://kc.example.com/realms/nitro"
    claims = {"iss": issuer, "aud": "nitro-ui", "exp": int(time.time()) + 300, "sub": "x", "nonce": "n"}
    token = _sign_id_token(pem, claims)
    with _patch_settings():
        with patch("app.services.oidc._fetch_jwks", return_value=jwks):
            with pytest.raises(ValueError):
...
def test_validate_id_token_rejects_nonce_mismatch():
    ...
    with _patch_settings():
        with patch("app.services.oidc._fetch_jwks", return_value=jwks):
            with pytest.raises(ValueError):
                oidc.validate_id_token(token, issuer=issuer, audience="nitro-ui", nonce="different")


def test_validate_id_token_rejects_expired():
    pem, jwks = _make_keypair_and_jwks()
    issuer = "https://kc.example.com/realms/nitro"
    claims = {"iss": issuer, "aud": "nitro-ui", "exp": int(time.time()) - 10, "sub": "x", "nonce": "n"}
    token = _sign_id_token(pem, claims)
    with _patch_settings():
        with patch("app.services.oidc._fetch_jwks", return_value=jwks):
            with pytest.raises(ValueError):
                oidc.validate_id_token(token, issuer=issuer, audience="nitro-ui", nonce="n")
