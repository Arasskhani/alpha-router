...
| alpha-router container | user=`alpha_router`, ReadonlyRootfs=true, CapDrop=ALL, no-new-privileges, **no** docker.sock |
...
- **Finding:** Unlike alpha-router (`user=alpha_router`), broker user empty (typically root) while holding docker.sock.
