...
vi.mock("../lib/mediaUrl", () => ({
    ...
    isAlphaRouterMediaFileUrl: (url) => /^\/api\/chat\/media\/\d+\/file$/.test(url),
}));
...
describe("authenticated Markdown media downloads", () => {
    it("fetches with the authenticated media helper and downloads the canonical filename", async () => {
        ...
        await downloadAuthenticatedMedia("/api/chat/media/42/file", "management-report.pdf");
        expect(mediaMocks.fetchAuthenticatedMediaBlob).toHaveBeenCalledWith("/api/chat/media/42/file");
