...
import { CODE_BLOCK_COLLAPSED_PREVIEW_LINES, codeBlockExpandStateKey, getCollapsedCodePreview, } from "./ChatCodeBlock";
...
describe("codeBlockExpandStateKey", () => {
    it("stays stable when only lines after the preview change", () => {
        ...
        const a = `${head}\ntail-a`;
        const b = `${head}\ntail-b\nmore`;
        expect(codeBlockExpandStateKey(a, "python", "code")).toBe(codeBlockExpandStateKey(b, "python", "code"));
    });
});
