...
export default function ConnectionChangelog({ connectionId }: Props) {
  ...
  useEffect(() => {
    ...
    api<{ items: ChangelogEntry[] }>(`/api/admin/connections/${connectionId}/changelog${suffix}`)
  ...
  return (
    <section className="card api-key-changelog">
      ...
      <p className="muted-text api-key-changelog__lead">
        Creation and edits to this provider connection (who changed what).
      </p>
      ...
      <div className="api-key-changelog__filters">
        <label className="api-key-changelog__filter">
          ...
          <input type="date" className="input-block" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} />
        </label>
        <label className="api-key-changelog__filter">
