...
describe("safeBrowserUrl", () => {
    it("allows same-origin paths and secure external navigation", () => {
        expect(safeBrowserUrl("/api/chat/media/1/file", "image", BASE)).toBe("/api/chat/media/1/file");
        expect(safeBrowserUrl("https://docs.example.com/page", "navigation", BASE)).toBe("https://docs.example.com/page");
        expect(safeBrowserUrl("mailto:user@example.com", "navigation", BASE)).toBe("mailto:user@example.com");
    });
    ...
    it("allows raster data images but blocks SVG and active data payloads", () => {
        expect(safeBrowserUrl("data:image/png;base64,AAAA", "image", BASE)).toBe("data:image/png;base64,AAAA");
        expect(safeBrowserUrl("data:image/svg+xml,<svg onload='alert(1)'></svg>", "image", BASE)).toBeNull();
