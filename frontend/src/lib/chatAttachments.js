...
import { ATTACHMENT_MESSAGE_PREFIX, AUDIO_MESSAGE_PREFIX, } from "./chatMarkers";
...
function fileExtensions(name) {
    const parts = name.toLowerCase().split(".");
    if (parts.length < 2)
        return [];
    return parts.slice(1);
}
export function attachmentKindFromName(name) {
...
export function readAttachmentMessage(content) {
    if (!content.startsWith(ATTACHMENT_MESSAGE_PREFIX))
        return null;
    try {
        return JSON.parse(content.slice(ATTACHMENT_MESSAGE_PREFIX.length));
    }
    catch {
        return null;
    }
}
export function attachmentDisplayText(payload) {
...
export function cloneProcessedAttachments(attachments) {
    return attachments.map((a) => ({ ...a }));
}
/** Drop inline image bytes from persisted chat JSON; media remains available via url. */
export function compactAttachmentMessageForStorage(content) {
...
export function compactChatMessagesForStorage(messages) {
    return messages.map((m) => m.role === "user" ? { ...m, content: compactAttachmentMessageForStorage(m.content) } : m);
}
export function referenceImageFromUserContent(content) {
    const attach = readAttachmentMessage(content);
    if (!attach)
        return undefined;
...
export async function resolveReferenceImageFromUserContent(content) {
...
export function promptTextFromUserContent(content) {
    const attach = readAttachmentMessage(content);
    if (attach)
        return attach.userText.trim();
    return content.trim();
}
/** True when the user likely wants to edit the previous generated image (not a fresh scene). */
export function promptImpliesImageEdit(prompt) {
    const text = prompt.trim();
    if (!text)
        return false;
    return IMAGE_EDIT_PROMPT_HINTS.some((rx) => rx.test(text));
}
export function shouldRouteToImageGeneration(userContent, imageGenerationEnabled, supportsTextToImage, supportsImageToImage, priorAssistantImageUrl) {
    ...
    if (attach) {
        ...
        if (ref)
            return supportsImageToImage;
        return false;
    }
    const prompt = promptTextFromUserContent(userContent);
    if (priorAssistantImageUrl &&
...
export async function processAttachmentFilesLocally(files) {
    const out = [];
...
export function modelSupportsVision(m) {
    ...
    if (isAutoRouterExternalId(m.external_id))
        return true;
    const v = `${m.external_id || ""} ${m.name || ""} ${m.id || ""}`.toLowerCase();
    if (isImageGenerationModel(v))
        return false;
    return (v.includes("vision") ||
        v.includes("gpt-4o") ||
        v.includes("gpt-4.1") ||
...
function isImageGenerationModel(v) {
    return (v.includes("dall") ||
        v.includes("flux") ||
        v.includes("sdxl") ||
...
export function imageUrlNeedsAuthResolve(url) {
...
export async function resolveAttachmentImageUrlForApi(raw) {
    ...
    if (!isAlphaRouterMediaFileUrl(trimmed))
        return trimmed;
...
export async function buildApiMessageContentAsync(content, visionModel) {
    ...
    if (audio) {
        try {
            const parsed = JSON.parse(content.slice(AUDIO_MESSAGE_PREFIX.length));
...
export function buildApiMessageContent(content, visionModel) {
    const audio = content.startsWith(AUDIO_MESSAGE_PREFIX);
    if (audio) {
        try {
            const parsed = JSON.parse(content.slice(AUDIO_MESSAGE_PREFIX.length));
...
export function hasApiContent(content) {
