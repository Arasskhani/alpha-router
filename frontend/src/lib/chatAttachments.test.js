...
import { ATTACHMENT_MESSAGE_PREFIX, AUDIO_MESSAGE_PREFIX, attachmentKindFromName, attachmentMessage, buildApiMessageContent, canProcessAttachmentLocally, imageUrlNeedsAuthResolve, modelSupportsVision, readAttachmentMessage, } from "./chatAttachments";
...
describe("modelSupportsVision", () => {
    ...
    it("falls back to heuristic for gemini when no flag is present", () => {
        expect(modelSupportsVision({ id: "model::1", external_id: "google/gemini-2.5-flash" })).toBe(true);
    });
    it("falls back to heuristic for claude when no flag is present", () => {
        expect(modelSupportsVision({ id: "model::2", external_id: "anthropic/claude-3.5-sonnet" })).toBe(true);
    });
    ...
    it("excludes image-generation models even via heuristic", () => {
        expect(modelSupportsVision({ id: "model::img", external_id: "black-forest-labs/flux-1.1-pro" })).toBe(false);
        expect(modelSupportsVision({ id: "model::dalle", external_id: "openai/dall-e-3" })).toBe(false);
    });
    it("returns false for a plain text model without the flag", () => {
        expect(modelSupportsVision({ id: "model::llama", external_id: "meta-llama/llama-3.1-70b-instruct" })).toBe(false);
    });
});
