...
export const ATTACHMENT_MESSAGE_PREFIX = "__NITRO_ATTACH_JSON__:";
...
export async function buildApiMessageContentAsync(
  ...
  const audio = content.startsWith("__NITRO_AUDIO_JSON__:");
  if (audio) {
    try {
      const parsed = JSON.parse(content.slice("__NITRO_AUDIO_JSON__:".length)) as { transcript?: string };
