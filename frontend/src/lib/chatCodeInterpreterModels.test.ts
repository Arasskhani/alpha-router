...
import {
  ...
  filterCodeInterpreterModels,
  findCodeInterpreterFallbackModel,
  isVerifiedCodeInterpreterModel,
  modelSupportsCodeInterpreter,
  resolveCodeInterpreterModel,
  type CodeInterpreterModelRef,
} from "./chatCodeInterpreterModels";

function model(
  id: string,
  external_id: string,
  compat?: Partial<CodeInterpreterModelRef["code_interpreter"]>,
): CodeInterpreterModelRef {
  return {
    id,
    name: id,
    external_id,
    code_interpreter: compat
      ? ({
          ...
          selectable: true,
          ...compat,
        } as CodeInterpreterModelRef["code_interpreter"])
      : undefined,
  };
}
...
describe("Code Interpreter model compatibility", () => {
  ...
  it("hides models the backend marked blocked or quarantined", () => {
    ...
    expect(codeInterpreterBlockedReason(blocked)).toBe(
      "Returns malformed tool calls.",
    );
  ...
  it("prefers a verified model, then Auto Router, then any allowed model", () => {
    ...
    expect(findCodeInterpreterFallbackModel([auto, plain, verified])?.id).toBe(
      "verified",
    );
