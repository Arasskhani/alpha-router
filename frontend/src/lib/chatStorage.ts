...
function legacyStorageKey(): string {
  ...
  return `nitro_chats_${sub}`;
}

function legacyFoldersStorageKey(): string {
  const token = localStorage.getItem("nitro_token");
  const sub = token ? token.slice(0, 12) : "anon";
  return `nitro_chat_folders_${sub}`;
}
...
const _IMAGE_PREFIX = "__NITRO_IMAGE_JSON__:";
const _IMAGE_PENDING = "__NITRO_IMAGE_PENDING__";
...
export function normalizeMessageForTitle(content: string): string {
  ...
  const attachPrefix = "__NITRO_ATTACH_JSON__:";
  ...
  const audioPrefix = "__NITRO_AUDIO_JSON__:";
  ...
  if (raw.startsWith("__BILLI_IMAGE_JSON__:")) return _parseImagePrompt(raw, "__BILLI_IMAGE_JSON__:");
  if (raw.startsWith("__NITRO_IMAGE__:") || raw.startsWith("__BILLI_IMAGE__:")) return "Generated image";
