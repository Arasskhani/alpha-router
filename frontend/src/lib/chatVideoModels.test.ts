...
describe("chatVideo routing", () => {
  it("routes when tool and model support video", () => {
    expect(
      shouldRouteToVideoGeneration({ videoGenerationEnabled: true, modelSupportsVideo: true }),
    ).toBe(true);
    expect(
      shouldRouteToVideoGeneration({ videoGenerationEnabled: false, modelSupportsVideo: true }),
    ).toBe(false);
  });
