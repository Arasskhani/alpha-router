...
import { attachMediaIds, attachSlotOverflowMessage, capAttachSelection, composerAttachEligibility, mediaBlobToFile, normalizeProjectMediaItem, normalizeUserMediaItem, } from "./composerAttachSources";
...
describe("composerAttachEligibility", () => {
    ...
    it("allows documents outside Private Mode", () => {
        expect(composerAttachEligibility(candidate({ kind: "document", fileName: "brief.pdf", mimeType: "application/pdf" }), { privateMode: false })).toEqual({
            attachable: true,
            reason: null,
    ...
    it("in Private Mode allows images and plain text only", () => {
        expect(composerAttachEligibility(candidate({ fileName: "note.txt", kind: "document", mimeType: "text/plain" }), {
            privateMode: true,
        }).attachable).toBe(true);
        expect(composerAttachEligibility(candidate({ kind: "document", fileName: "brief.pdf", mimeType: "application/pdf" }), { privateMode: true }).attachable).toBe(false);
    });
});
...
describe("attachMediaIds", () => {
    it("posts unique positive ids to from-media", async () => {
        ...
        try {
            ...
            expect(spy).toHaveBeenCalledWith("/api/chat/attachments/from-media", expect.objectContaining({ method: "POST" }));
