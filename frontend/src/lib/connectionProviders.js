...
export const CONNECTION_PROVIDER_PRESETS = [
    {
        id: "openrouter",
        label: "OpenRouter",
...
function presetMatches(preset, query) {
    if (!query)
        return true;
    if (preset.id.includes(query))
        return true;
    if (preset.label.toLowerCase().includes(query))
        return true;
...
export function suggestConnectionProviders(query, limit = 8) {
    const q = normalizeQuery(query);
    const scored = CONNECTION_PROVIDER_PRESETS.map((preset) => {
        if (!presetMatches(preset, q))
            return null;
        let score = 0;
        if (!q)
            score = 1;
            ...
            score = 100;
        else if ((preset.aliases ?? []).includes(q))
            score = 95;
        else if (preset.id.startsWith(q))
            score = 80;
        else if (preset.label.toLowerCase().startsWith(q))
            score = 70;
        else if (preset.id.includes(q))
            score = 50;
        else
            score = 20;
        return { preset, score };
...
export function resolveConnectionProvider(query) {
    const q = normalizeQuery(query);
    if (!q)
        return null;
    const exact = CONNECTION_PROVIDER_PRESETS.find((preset) => preset.id === q ||
        preset.label.toLowerCase() === q ||
        (preset.aliases ?? []).includes(q));
    return exact ?? null;
}
export function knownConnectionBaseUrls() {
    return new Set(CONNECTION_PROVIDER_PRESETS.map((preset) => preset.baseUrl.trim()).filter(Boolean));
}
