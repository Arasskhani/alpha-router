...
describe("suggestedEmbeddingDimensions", () => {
  ...
  it("formats the option label with dimensions", () => {
    expect(embeddingModelOptionLabel("google/gemini-embedding-001", "openrouter")).toBe(
      "google/gemini-embedding-001 · openrouter · 3072d",
    );
  });
});
...
describe("embeddingModelIdFromSpec", () => {
  it("strips the provider prefix", () => {
    expect(embeddingModelIdFromSpec("openrouter:google/gemini-embedding-001")).toBe(
