...
export const DEFAULT_IMAGE_ASPECT_PRESET = "square";
...
const PRESET_BY_ID = Object.fromEntries(IMAGE_ASPECT_PRESETS.map((p) => [p.id, p]));
const ASPECT_RATIO_TO_PRESET = {
    "1:1": "square",
    "16:9": "wide",
    "9:16": "tall",
    "3:2": "landscape",
...
const ASPECT_RATIO_TO_DEFAULT_SIZE = {
    "1:1": "1024x1024",
    "16:9": "1344x768",
    "9:16": "768x1344",
    "3:2": "1248x832",
...
export function normalizeCustomAspectRatio(raw) {
    if (!raw)
        return null;
    ...
    if (SUPPORTED_ASPECT_RATIOS.includes(simplified)) {
        return simplified;
    }
    return snapAspectRatio(w, h);
}
/** Migrate legacy WxH session values (e.g. 1280x720) to aspect ratio. */
export function aspectRatioFromLegacySize(size) {
    if (!size)
        return null;
...
export function normalizeImageAspectPreset(raw) {
    if (raw === "custom")
        return "custom";
    if (raw && raw in PRESET_BY_ID)
        return raw;
    return DEFAULT_IMAGE_ASPECT_PRESET;
}
export function aspectRatioFromPreset(preset, customAspect) {
    if (preset === "custom") {
        return normalizeCustomAspectRatio(customAspect) ?? DEFAULT_CUSTOM_ASPECT_RATIO;
    }
    return PRESET_BY_ID[preset].aspectRatio;
}
export function presetFromAspectRatio(aspectRatio) {
    if (!aspectRatio)
        return undefined;
...
export function aspectLabelForPreset(preset, customAspect) {
    if (preset === "custom") {
        return normalizeCustomAspectRatio(customAspect) ?? DEFAULT_CUSTOM_ASPECT_RATIO;
    }
    return PRESET_BY_ID[preset].shortLabel;
}
export function defaultSizeForAspectRatio(aspectRatio) {
...
export function resolveSessionAspectRatio(tools) {
    ...
    if (preset === "custom") {
        const fromCustom = normalizeCustomAspectRatio(tools.imageCustomAspectRatio) ??
...
function applyAspectToken(token, found) {
...
export function parseAspectRatioFromPrompt(prompt) {
    let text = prompt.trim();
    const found = {};
    text = text.replace(/--(?:ar|aspect(?:-ratio)?)\s*[:=]?\s*(\d+\s*:\s*\d+|\d+\s*x\s*\d+)/gi, (_, token) => {
        if (!found.aspectRatio)
            applyAspectToken(token, found);
        return " ";
    });
    if (!found.aspectRatio) {
        text = text.replace(/\b(16:9|9:16|1:1|3:2|2:3|4:3|3:4|21:9)\b/g, (match) => {
            if (!found.aspectRatio)
                applyAspectToken(match, found);
            return " ";
        });
    }
...
export function resolveRegenerateImageGeneration(opts) {
    ...
    const fromPayloadRatio = opts.payloadAspectRatio ??
        (opts.payloadSize ? aspectRatioFromLegacySize(opts.payloadSize) : undefined);
    const fromPayload = opts.payloadPreset ?? (fromPayloadRatio ? presetFromAspectRatio(fromPayloadRatio) : undefined);
...
export function readReferenceImageDimensions(referenceUrl) {
