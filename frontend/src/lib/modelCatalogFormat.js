...
export function formatModelTitle(m) {
    if (m.title && m.title !== m.external_id)
        return m.title;
    if (m.display_name && m.display_name !== m.external_id)
...
export function formatPerMTokenPrice(per1k, kind) {
    const perM = per1k * 1000;
    const label = kind === "input" ? "input" : "output";
    if (perM >= 1)
        return `$${perM.toFixed(2)}/M ${label} tokens`;
    if (perM >= 0.01)
        return `$${perM.toFixed(2)}/M ${label} tokens`;
    return `$${perM.toFixed(4)}/M ${label} tokens`;
}
export function formatReleasedDate(iso) {
