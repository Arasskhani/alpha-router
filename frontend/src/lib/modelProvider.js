...
export function providerFromModelId(modelId) {
    ...
    if (raw.includes("mistral") || raw.includes("mixtral"))
        return "mistral";
    if (raw.includes("runway"))
        return "runway";
    if (raw.includes("llama"))
        return "meta";
    if (raw.includes("gpt") || /(^|\/)o[1-4]/.test(raw))
        return "openai";
...
export function resolveProviderIconSlug(modelId, provider) {
    ...
    if (rawProvider) {
        if (rawProvider.includes("gemini"))
            return "gemini";
