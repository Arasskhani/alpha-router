...
import { buildImageMessage, IMAGE_MESSAGE_PREFIX } from "./chatImage";
import { isPrivateBlobRef, resolvePrivateBlobRef, storePrivateMediaUrl, } from "./privateMediaStore";
...
function shouldOffloadMediaUrl(url) {
    if (!url)
        return false;
    const trimmed = url.trim();
    if (!trimmed || isPrivateBlobRef(trimmed))
        return false;
    return trimmed.startsWith("data:") || trimmed.startsWith("blob:");
}
async function compactMediaUrl(url) {
    if (!url?.trim())
        return url;
    if (!shouldOffloadMediaUrl(url))
        return url;
    return storePrivateMediaUrl(url);
}
async function compactPrivateImageContent(content) {
    ...
    if (url === payload.url && reference_image === payload.reference_image)
        return content;
    return buildImageMessage({ ...payload, url: url || payload.url, reference_image });
}
async function compactPrivateAttachmentContent(content) {
...
async function hydratePrivateImageContent(content) {
    const payload = parseImageMessage(content);
    if (!payload)
        return content;
...
async function hydratePrivateAttachmentContent(content) {
    const attach = readAttachmentMessage(content);
    if (!attach)
        return content;
    let changed = false;
...
async function hydratePrivateMessageContent(content) {
    if (content.startsWith(IMAGE_MESSAGE_PREFIX)) {
...
export async function compactPrivateSessionsForStorage(sessions) {
    const out = [];
...
export async function hydratePrivateSessionsFromStorage(sessions) {
    const out = [];
    for (const session of sessions) {
        if (!session.privateMode) {
            out.push(session);
...
export function privateSessionsNeedStorageMigration(sessions) {
    for (const session of sessions) {
        if (!session.privateMode)
            continue;
        for (const msg of session.messages) {
            const image = parseImageMessage(msg.content);
            if (image && shouldOffloadMediaUrl(image.url))
                return true;
            if (image?.reference_image && shouldOffloadMediaUrl(image.reference_image))
                return true;
            const attach = readAttachmentMessage(msg.content);
            if (attach) {
                for (const att of attach.attachments) {
                    if (att.kind === "image" && shouldOffloadMediaUrl(att.data_url || att.url))
                        return true;
                }
            }
        }
