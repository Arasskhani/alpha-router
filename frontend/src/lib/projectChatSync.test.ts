...
import {
  applyProjectChatSync,
  applyProjectPins,
  mergeIncomingProjectMessages,
  mergeProjectMessage,
  pickProjectSyncMessageBase,
  sortProjectSessions,
} from "./projectChatSync";
...
describe("pickProjectSyncMessageBase", () => {
  it("uses the on-screen thread when the sidebar row is empty", () => {
    const displayed = [
      { id: "1", role: "user" as const, content: "hi", sequence: 1 },
      { id: "2", role: "assistant" as const, content: "yo", sequence: 2, receivedAt: 9 },
    ];
  ...
  it("keeps a longer sidebar copy so a prior merge is not discarded", () => {
    const sessionMsgs = [
      { id: "1", role: "user" as const, content: "hi", sequence: 1 },
      { id: "2", role: "assistant" as const, content: "yo", sequence: 2 },
      { id: "3", role: "user" as const, content: "again", sequence: 3 },
    ];
...
describe("mergeProjectMessage", () => {
  it("does not replace local assistant text with an empty persist placeholder", () => {
    ...
    const incoming = { id: "a1", role: "assistant" as const, content: "", sequence: 2 };
...
describe("mergeIncomingProjectMessages", () => {
  it("appends incremental messages by id", () => {
    const local = [{ id: "1", role: "user" as const, content: "hi", sequence: 1 }];
    const incoming = [{ id: "2", role: "assistant" as const, content: "yo", sequence: 2 }];
  ...
  it("updates patched assistant content in place", () => {
    const local = [
      { id: "1", role: "user" as const, content: "hi", sequence: 1 },
      { id: "2", role: "assistant" as const, content: "", clientMessageId: "a1", sequence: 2 },
    ];
    const incoming = [
      { id: "2", role: "assistant" as const, content: "hello world", clientMessageId: "a1", sequence: 2 },
    ];
    const merged = mergeIncomingProjectMessages(local, incoming, true);
  ...
  it("keeps local text when the incremental patch is still an empty placeholder", () => {
    const local = [
      { id: "1", role: "user" as const, content: "hi", sequence: 1 },
      { id: "2", role: "assistant" as const, content: "already streamed", sequence: 2 },
    ];
    const incoming = [{ id: "2", role: "assistant" as const, content: "", sequence: 2 }];
    const merged = mergeIncomingProjectMessages(local, incoming, true);
...
describe("applyProjectChatSync", () => {
  it("applies live pins and new sessions", () => {
    ...
    const { sessions } = applyProjectChatSync(
      local,
      {
        serverTimeMs: 10,
        total: 2,
  ...
  it("applies patched assistant content for the open thread", () => {
    const local = [
      session({
        id: "open",
        messages: [
          { id: "u1", role: "user" as const, content: "hi", sequence: 1 },
          { id: "a1", role: "assistant" as const, content: "", clientMessageId: "c-a", sequence: 2 },
        ],
      }),
    ];
    const { sessions, activeMessages } = applyProjectChatSync(
      local,
      {
        serverTimeMs: 10,
        total: 1,
        ...
        messages: [
          { id: "a1", role: "assistant" as const, content: "done", clientMessageId: "c-a", sequence: 2 },
        ],
  ...
  it("merges an incremental tail onto the displayed thread when the sidebar row is empty", () => {
    const displayed = [
      { id: "u1", role: "user" as const, content: "hi", sequence: 1, receivedAt: 1 },
      { id: "a1", role: "assistant" as const, content: "old", sequence: 2, receivedAt: 2 },
    ];
    const { activeMessages, sessions } = applyProjectChatSync(
      [session({ id: "open", messages: [] })],
      {
        serverTimeMs: 10,
        total: 1,
        pinnedSessionIds: [],
        ...
        messages: [
          { id: "a1", role: "assistant" as const, content: "old", sequence: 2 },
          { id: "u2", role: "user" as const, content: "again", sequence: 3 },
          { id: "a2", role: "assistant" as const, content: "new reply", sequence: 4 },
        ],
        messageSessionId: "open",
        completeWindow: true,
      },
  ...
  it("does not skip message patches when the session is only marked streaming in the UI", () => {
    const local = [
      session({
        ...
        messages: [
          { id: "u1", role: "user" as const, content: "hi", sequence: 1 },
          { id: "a1", role: "assistant" as const, content: "", sequence: 2 },
        ],
      }),
    ];
    const { activeMessages } = applyProjectChatSync(
      local,
      {
        serverTimeMs: 10,
        total: 1,
        ...
        sessionIds: ["open"],
        sessions: [session({ id: "open", updatedAt: 20, lastMessageAt: 20 })],
        messages: [{ id: "a1", role: "assistant" as const, content: "done", sequence: 2, receivedAt: 9 }],
        messageSessionId: "open",
        completeWindow: true,
      },
      {
  ...
  it("still skips message patches while this tab is the local writer", () => {
    const local = [
      session({
        ...
        messages: [
          { id: "u1", role: "user" as const, content: "hi", sequence: 1 },
          { id: "a1", role: "assistant" as const, content: "local stream", sequence: 2 },
        ],
      }),
    ];
    const { activeMessages, sessions } = applyProjectChatSync(
      local,
      {
        ...
        sessionIds: ["open"],
        sessions: [session({ id: "open", updatedAt: 20, lastMessageAt: 20 })],
        messages: [{ id: "a1", role: "assistant" as const, content: "", sequence: 2 }],
        messageSessionId: "open",
        completeWindow: true,
      },
      {
