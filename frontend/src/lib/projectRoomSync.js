export function sortProjectRooms(rooms) {
...
export function mergeIncomingRoomMessages(local, incoming, incremental) {
    if (!incoming.length)
        return local;
    if (!local.length)
        return incoming;
...
export function applyProjectRoomSync(local, sync, opts) {
    ...
    if (opts.activeRoomId && sync.messageSessionId === opts.activeRoomId && sync.messages) {
        activeMessages = mergeIncomingRoomMessages(opts.activeLocalMessages ?? [], sync.messages, Boolean(opts.incrementalMessages));
    }
