...
import {
  applyProjectRoomSync,
  mergeIncomingRoomMessages,
  sortProjectRooms,
} from "./projectRoomSync";

function room(partial: Partial<ProjectRoom> & { id: string }): ProjectRoom {
  return {
    title: "Room",
    messageCount: 0,
    revision: 1,
...
function msg(partial: Partial<ProjectRoomMessage> & { id: string }): ProjectRoomMessage {
  return {
    role: "user",
    content: "",
    ...partial,
...
describe("applyProjectRoomSync", () => {
  it("merges rooms and drops gone ids", () => {
    ...
    const sync: ProjectRoomSyncPayload = {
      serverTimeMs: 10,
      total: 1,
      sessionIds: ["keep"],
