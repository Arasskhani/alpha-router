...
import { groundingPolicyFromToggles, groundingTogglesFromPolicy, assignableProjectRoles, isPrimaryOwnerRole, isProjectOwnerRole, MAX_PROJECT_RESOURCE_UPLOAD_FILES, needsPublicTypedConfirm, projectResourceKnowledgeStatus, projectResourceTooManyFilesMessage, projectResourceUserStatus, projectRoleLabel, queueProjectMediaForChat, } from "./projectsApi";
const item = {
    id: 7,
    projectId: "proj-abc",
    kind: "image",
...
describe("groundingPolicyFromToggles", () => {
    it("serializes toggle flags into grounding_policy keys", () => {
        expect(groundingPolicyFromToggles({ useProjectResources: false, useGrantedMemory: true })).toEqual({
...
describe("project resource upload limits", () => {
    it("uses the same 20-file cap and message as Knowledge Bases", () => {
        ...
        expect(projectResourceTooManyFilesMessage(21)).toBe("You can upload at most 20 files at a time. You selected 21. Please choose 20 or fewer and try again.");
    });
    it("prefers Knowledge version status for display", () => {
        expect(projectResourceKnowledgeStatus({
            id: "r1",
            projectId: "p1",
            documentId: "d1",
            title: "Doc",
    ...
    it("shows Waiting for Admin Approval until the resource is Published", () => {
        ...
        expect(projectResourceUserStatus({ ...base, versionStatus: "quarantined" })).toBe("Waiting for Admin Approval");
        expect(projectResourceUserStatus({ ...base, versionStatus: "processing" })).toBe("Waiting for Admin Approval");
        expect(projectResourceUserStatus({ ...base, versionStatus: "review" })).toBe("Waiting for Admin Approval");
        expect(projectResourceUserStatus({ ...base, status: "active", versionStatus: "published" })).toBe("Published");
        expect(projectResourceUserStatus({ ...base, status: "failed", versionStatus: "failed" })).toBe("Failed");
    });
});
