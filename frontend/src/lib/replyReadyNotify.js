...
function pruneDedupe(now) {
    for (const [key, at] of recentKeys) {
        if (now - at > DEDUPE_TTL_MS * 4)
            recentKeys.delete(key);
    }
}
function shouldDedupe(sessionId, messageKey) {
    const now = Date.now();
    pruneDedupe(now);
    const key = `${sessionId}::${messageKey}`;
    const prev = recentKeys.get(key);
    if (prev != null && now - prev < DEDUPE_TTL_MS)
        return true;
    // Also coalesce per-session briefly (multi-model bursts).
    const sessionKey = `session::${sessionId}`;
    const sessionPrev = recentKeys.get(sessionKey);
    if (sessionPrev != null && now - sessionPrev < DEDUPE_TTL_MS)
        return true;
    recentKeys.set(key, now);
    recentKeys.set(sessionKey, now);
    return false;
}
export function isUserAwayFromSession(sessionId, activeId) {
...
function dispatchFocusSession(sessionId) {
    window.dispatchEvent(new CustomEvent(REPLY_READY_FOCUS_EVENT, { detail: { sessionId } }));
    try {
        window.focus();
    }
    catch {
...
function showInAppToast(sessionId, title) {
...
function showOsNotification(sessionId, title) {
 ...
 */
export async function requestReplyNotifyPermission() {
...
export function notifyReplyReady(args) {
    const sessionId = (args.sessionId || "").trim();
    const messageKey = (args.messageKey || "").trim() || "unknown";
    if (!sessionId)
        return;
    if (!isUserAwayFromSession(sessionId, args.activeId))
        return;
    if (shouldDedupe(sessionId, messageKey))
        return;
