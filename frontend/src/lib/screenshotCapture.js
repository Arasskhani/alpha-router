...
export function normalizeDragRect(a, b) {
    const x = Math.min(a.x, b.x);
    const y = Math.min(a.y, b.y);
    return { x, y, width: Math.abs(b.x - a.x), height: Math.abs(b.y - a.y) };
}
export function containFit(imageWidth, imageHeight, boxWidth, boxHeight) {
...
export function mapDisplayedRectToImage(displayRect, fit, imageWidth, imageHeight) {
...
export function isCropLargeEnough(rect, min = MIN_CROP_PX) {
    return rect.width >= min && rect.height >= min;
}
export function screenshotFileName(at = new Date()) {
...
export function revokeScreenshotFrame(frame) {
    if (frame?.objectUrl)
        URL.revokeObjectURL(frame.objectUrl);
}
export function screenshotPermissionErrorMessage(err) {
...
async function waitForVideoFrame(video) {
    ...
    await new Promise((resolve, reject) => {
        const timeout = window.setTimeout(() => reject(new Error("Could not read the captured screen.")), 8000);
        const ok = () => {
...
export async function captureDisplayFrame() {
    ...
    try {
        ...
        if (!ctx)
            throw new Error("Could not capture a screenshot frame.");
        ctx.drawImage(video, 0, 0);
        const blob = await new Promise((resolve, reject) => {
            canvas.toBlob((next) => (next ? resolve(next) : reject(new Error("Could not encode the screenshot."))), "image/png");
        });
...
export async function cropScreenshotToFile(frame, imageRect, fileName) {
    const img = new Image();
