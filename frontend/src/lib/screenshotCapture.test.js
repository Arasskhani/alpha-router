...
import { containFit, isCropLargeEnough, mapDisplayedRectToImage, MIN_CROP_PX, normalizeDragRect, screenshotFileName, screenshotPermissionErrorMessage, } from "./screenshotCapture";
...
describe("containFit + mapDisplayedRectToImage", () => {
    it("maps a selection on a letterboxed image back to pixel coordinates", () => {
        ...
        const mapped = mapDisplayedRectToImage({ x: 40, y: 120, width: 80, height: 40 }, fit, 200, 100);
    ...
    it("clamps a selection that starts outside the image", () => {
        ...
        const mapped = mapDisplayedRectToImage({ x: -10, y: -5, width: 30, height: 20 }, fit, 100, 100);
        expect(mapped).toEqual({ x: 0, y: 0, width: 20, height: 15 });
    });
    it("returns null for an empty mapped crop", () => {
        const fit = containFit(100, 100, 100, 100);
        expect(mapDisplayedRectToImage({ x: 200, y: 200, width: 10, height: 10 }, fit, 100, 100)).toBe(null);
    });
});
...
describe("screenshotFileName", () => {
    it("uses a stable screenshot-YYYYMMDD-HHMMSS.png shape", () => {
        expect(screenshotFileName(new Date(2026, 7, 31, 21, 14, 5))).toBe("screenshot-20260831-211405.png");
    });
});
