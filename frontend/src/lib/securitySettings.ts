...
export function isReservedHttpsPort(port: number): boolean {
  if (!Number.isInteger(port) || port < 1 || port > 65535) return false;
  return (RESERVED_HTTPS_PORTS as readonly number[]).includes(port);
}

export function httpsHealthUrl(host: string, port: number): string {
  const hostname = host.trim() || "127.0.0.1";
  if (port === 443) return `https://${hostname}/health`;
  return `https://${hostname}:${port}/health`;
}

export function parseCidrInput(raw: string): { ok: true; cidr: string } | { ok: false; error: string } {
  const value = raw.trim();
  if (!value) return { ok: false, error: "Enter an IP address or CIDR range." };
...
export function canEnableEnforce(args: {
  ...
  entries: Array<Pick<AllowlistEntry, "cidr" | "enabled">>;
...
export function expiryBannerLevel(days: number | null): "none" | "warning" | "critical" | "expired" {
...
export function cidrContainsIp(cidr: string, ip: string): boolean {
  ...
  if (!parsed || !addr) return false;
  if (parsed.version !== addr.version) return false;
  const mask = parsed.prefix;
  if (mask < 0 || mask > (parsed.version === 4 ? 32 : 128)) return false;
...
function ipv6Literal(value: string): boolean {
  return value.includes(":");
}

function normalizeWildcard(cidr: string): string {
  return cidr.trim().toLowerCase();
}

function isPlausibleCidr(value: string): boolean {
...
function splitCidr(value: string): { network: bigint; prefix: number; version: 4 | 6 } | null {
...
function parseIp(value: string): { value: bigint; version: 4 | 6 } | null {
