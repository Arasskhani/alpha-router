...
export function getSessionUser() {
    ...
    const display_name = typeof session.display_name === "string" && session.display_name.trim()
...
export function formatSessionDuration(loginAt) {
...
export function setSessionActive(isActive) {
...
export async function logout() {
    ...
    if (localStorage.getItem(STORAGE_KEYS.privatePersist) !== "1") {
        for (const key of Object.keys(localStorage)) {
            if (key.startsWith(`${STORAGE_KEYS.privateChats}:`))
                localStorage.removeItem(key);
        }
        for (const key of Object.keys(sessionStorage)) {
            if (key.startsWith(`${STORAGE_KEYS.privateChats}:msgcache:`))
