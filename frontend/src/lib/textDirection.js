...
function stripMarkdownForDirection(text) {
    return text
        .replace(/```[\s\S]*?```/g, " ")
        .replace(/`[^`]+`/g, " ")
...
export function inputDirectionForText(text, caretIndex) {
...
export function messageDirectionForText(text) {
...
export function textNeedsEnglishTranslation(text) {
    const sample = stripMarkdownForDirection(text.trim());
    if (!sample)
        return false;
    if (RTL_CHAR.test(sample))
        return true;
    let latin = 0;
    let otherLetters = 0;
    for (const ch of sample) {
