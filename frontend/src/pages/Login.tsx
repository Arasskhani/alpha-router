...
export default function Login() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        localStorage.setItem("nitro_token", data.access_token);
        localStorage.setItem("nitro_role", normalizeRole(data.role));
        localStorage.setItem("nitro_auth_provider", "keycloak");
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      localStorage.setItem("nitro_token", data.access_token);
      localStorage.setItem("nitro_role", normalizeRole(data.role));
      localStorage.setItem("nitro_auth_provider", "local");
      const active = data.is_active !== false;
      markLoggedIn(active);
      if (isAdminPanelRole(data.role)) {
        const role = normalizeRole(data.role);
  ...
  return (
    <div className="login-page">
      ...
      <div className="login-page__shell">
        <section className="login-brand" aria-label="NITRO">
          <div className="login-brand__column">
            <div className="login-brand__head">
              <h1 className="login-brand__title login-brand__title--boost">
                <span className="login-brand__nitro-track" aria-hidden />
                <NitroLogo size={44} className="nitro-logo--gradient" />
              </h1>
