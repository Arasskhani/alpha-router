...
export default function AdminLayout() {
    ...
    const readOnly = useMemo(() => {
        if (session?.is_active === false)
            return true;
        if (isAdminUserFeaturePath(loc.pathname))
            return false;
        return !userCanWriteAdminPath(loc.pathname, session, role);
    }, [loc.pathname, session, role]);
    return (_jsx(ReadOnlyProvider, { value: readOnly, children: _jsx(AdminPermissionGuard, { nav: nav, session: session, children: _jsx(Shell, { nav: nav }) }) }));
}
